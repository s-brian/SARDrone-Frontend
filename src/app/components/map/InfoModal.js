  ...
  const handleImageLoad = () => {
    ...
    try {
      if (typeof log.bounding_boxes === "string" && log.bounding_boxes.trim()) {
        ...
        console.warn("Bounding boxes not a string or is empty:", log.bounding_boxes);
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] max-h-[90vh] !items-start !block overflow-hidden bg-gray-50">
        <DialogHeader className="pb-2 bg-white shadow-sm">
          <DialogTitle className="text-xl font-semibold text-gray-800">Log Details</DialogTitle>
        </DialogHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white rounded-xl p-4 shadow-sm">
                  <div className="flex items-center gap-2 mb-3">
                    <div className={`h-2 w-2 rounded-full ${log.score > 50 ? 'bg-red-500' : 'bg-gray-400'}`}></div>
                    <h3 className="font-medium text-gray-700">Human Detection</h3>
                  </div>
                ...
                <div className="bg-white rounded-xl p-4 shadow-sm">
                  ...
                  <div className="space-y-1">
                    <p className="text-gray-600">
                      <span className="text-gray-500">Lat:</span> {log.location.latitude.toFixed(6)}
                    </p>
                    <p className="text-gray-600">
                      <span className="text-gray-500">Long:</span> {log.location.longitude.toFixed(6)}
                    </p>
                    <p className="text-gray-600">
                      <span className="text-gray-500">Alt:</span> {log.location.altitude.toFixed(1)}m
                    </p>
                  </div>
                </div>
              </div>
